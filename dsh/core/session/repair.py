    ...
    if etype == "assistant/message":
        if "message" in data:
            ...
            if isinstance(msg, dict) and "id" not in msg:
                msg = {**msg, "id": legacy_message_id(session_id, seq)}
                return {**event, "data": {**data, "message": msg}}
            return event
    ...
    if etype == "tool/result":
        if "message" in data:
            ...
            if isinstance(msg, dict) and "id" not in msg:
                ...
                return {**event, "data": {**data, "message": msg}}
            return event
        if "callId" not in data or "content" not in data or "isError" not in data:
            return event
...
def migrate_legacy_event(event: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ev = migrate_legacy_steering_event(event, session_id)
    ev = migrate_legacy_turn_start_event(ev, session_id)
