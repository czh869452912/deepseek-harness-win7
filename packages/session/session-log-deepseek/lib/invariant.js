...
function validateDeliveryAccepted(session, event, fail) {
	...
	const inherited = session.header.parentSession !== void 0 && session.header.seedLength !== void 0 && event.seq < session.header.seedLength;
	...
	if (!Number.isSafeInteger(throughSeq) || throughSeq < 0 || throughSeq >= event.seq) fail(`session-log-deepseek/delivery-accepted throughSeq must identify an earlier event, got ${throughSeq} at seq ${event.seq}`);
}
/** Validate acceptance watermarks already present in one Session. */
function validateSession(session, fail) {
	for (const event of session.events) if (event.type === "session-log-deepseek/delivery-accepted") validateDeliveryAccepted(session, event, fail);
}
