...
async function selectJournalMode(db, path, journalMode, deadline) {
    ...
    while (true) {
        try {
            ...
            if (!isSqliteBusy(error) || remainingMs === 0)
                throw error;
...
export function decodeSessionRow(value) {
    ...
    if (cwd !== null && !isAbsolute(cwd))
        throw new Error('stored session cwd must be absolute');
    const parent = nullableStringField(row, 'parent_session');
    const origin = nullableStringField(row, 'origin');
    if (origin !== null && origin !== 'subagent')
        throw new Error('stored session origin must be subagent or null');
    const incarnation = nonemptyStringField(row, 'incarnation');
    if (!UUID.test(incarnation))
        throw new Error('stored session incarnation must be a UUID');
    return {
        id,
        version,
...
export function decodeStoreIdentity(value) {
    ...
    if (!UUID.test(identity))
        throw new Error('stored store_id must be a UUID');
    return identity;
}
/**
...
function stringField(value, key) {
    ...
    if (typeof field !== 'string')
        throw new Error(`stored ${key} must be a string`);
    return field;
}
function nonemptyStringField(value, key) {
    const field = stringField(value, key);
    if (field.length === 0)
        throw new Error(`stored ${key} must not be empty`);
    return field;
}
function nullableStringField(value, key) {
    ...
    if (field === null)
        return null;
    if (typeof field !== 'string')
        throw new Error(`stored ${key} must be a string or null`);
    return field;
}
function stringOrBlobField(value, key) {
    const field = record(value, 'SQLite row')[key];
    if (typeof field === 'string' || field instanceof Uint8Array)
        return field;
    throw new Error(`stored ${key} must be a string or blob`);
}
function nullableBlobField(value, key) {
    const field = record(value, 'SQLite row')[key];
    if (field === null || field instanceof Uint8Array)
        return field;
...
function nonnegativeSafeIntegerField(value, key) {
    ...
    if (field < 0)
        throw new Error(`stored ${key} must be non-negative`);
    return field;
}
...
function nullableNonnegativeSafeIntegerField(value, key) {
    ...
    if (field !== null && field < 0)
        throw new Error(`stored ${key} must be non-negative or null`);
    return field;
}
