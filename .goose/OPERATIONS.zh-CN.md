...
- 主审（含集成审查）显式使用 `thinking_effort: medium`，仲裁使用 `high`。可在角色配置中选择 low/medium/high；省略则继承 Goose 设置。控制器通过每个子进程独立的 `GOOSE_THINKING_EFFORT` 传入，不修改全局环境；强度变化会使旧阶段缓存失效。Sol 的 provider 上下文限制仍为 272000。
...
`integrate` 是保留候选上的修复阶段，不是合入主线；`MUST_FIX` 可以进入该阶段，但不能因此通过最终合入门禁。

- 集成审查接入问题台账、同问题复现检测、每三轮未收敛仲裁和 ESCALATE 仲裁。没有仲裁历史的旧运行，在第三轮及以后仍未通过时，恢复后先仲裁再修复。无法给出明确裁决则停在 NEEDS_ARBITRATION。
- 集成审查只接收先前审查、实际影响范围和仲裁证据，不接收实现者自述。审查模型或验收范围变化时重新建立完整覆盖。
- 子进程保留外部代理，仅为 localhost、127.0.0.1 和 IPv6 loopback 补齐 NO_PROXY。pytest 非断言失败退出码进入 FAILED_INFRA；恢复时重跑集成门禁，不先改产品代码。同一全量失败用例再次出现时，要求仲裁后再修。
- 集成修复输入按轮冻结，暂停恢复可复用已完成结果；影响路径累计保留；计划提案进入 WAITING_PLAN，冲突进入 PLAN_REPAIR。
- 新报告要求每个 issue 显式提供 state。完成后格式不合规时，在原会话只重排报告一次，禁止编辑文件和重跑测试；仍无效则保留现场报错，不推断 PASS。历史缺失 state 的问题仍保守视作 open。
- 目标测试去除父目录已覆盖的子路径，失败反馈保留具体退出码和日志。全量测试由控制器最终门禁运行，integrator 不应借环境失败扩大产品修复范围。

这些措施约束重复劳动和错误路由，不保证复杂 JS/Python 异步语义一次翻译完成；是否保持监听器同步前缀顺序、Promise continuation 时序等，仍需可复现测试和源代码证据。
