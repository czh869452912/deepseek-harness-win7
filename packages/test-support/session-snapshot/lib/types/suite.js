...
import { parseSnapshotManifest } from "./manifest.js";
import { redactSessionSnapshotIds } from "./identity.js";
import { captureExpectedWorkspaceSnapshot } from "./workspace.js";
import { extractSnapshotSpillPaths, normalizeSessionLog, normalizeSessionSnapshots, normalizeStdout, scrubRequestHeaders, scrubSessionSnapshot, scrubSystemPrompts, scrubToolSchemas, tokenizeSessionFixtureCwd, } from "./normalize.js";
            ...
            it.skipIf(scenarioSkipped(scenario, RECORDING, process.platform, options.hasPwsh))(`snapshot: ${scenario.name} matches the expected outputs`, async ({ expect }) => {
                ...
                for (const [logIndex, log] of result.sessionLogs.entries()) {
                    ...
                    if (scenario.pinsHeader === true && logIndex === 0) {
                        ...
                        expect(formatSystemPromptSnapshot(pinningPrompts[0], pinningPrompts.slice(1)), `session ${log.id}: changed system prompts diverged from ${promptSource.name}/${SYSTEM_PROMPT_SNAPSHOT}`)
