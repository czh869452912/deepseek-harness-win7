 ...
 */
export function redactSessionSnapshotIds(logs) {
    const parsed = logs.map(parseLog);
    const tokenByValue = new Map();
    const nextByKind = new Map();
    const claim = (value, kind, always = false) => {
        if (typeof value !== 'string' || value.length === 0 || tokenByValue.has(value))
            return;
        if (!always && !redactedCandidate(value))
            return;
    ...
    for (const log of parsed) {
        ...
        if (header?.type === 'session')
            claim(header.id, 'session', true);
    }
    const collect = (value, recordType) => {
        if (typeof value === 'string') {
            for (const match of value.matchAll(/\bas message ([0-9a-f-]{36})\b/gi))
                claim(match[1], 'message');
            for (const match of value.matchAll(/\bAnonymous user: ([0-9a-f-]{36})\b/gi))
                claim(match[1], 'id');
            return;
        }
        ...
        if (identifiedMessage !== undefined)
            claim(identifiedMessage, 'message');
